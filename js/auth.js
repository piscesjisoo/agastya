...
const Auth = (() => {
  ...
  async function login(username, password) {
    // ═══════════════════════════════════════════════════════════
    // 🔐 기본 admin 계정 — DB 없어도 무조건 로그인 가능 (하드코딩)
    // 데이터 초기화·재배포·DB 장애 시에도 admin 접근 보장
    // ═══════════════════════════════════════════════════════════
    if (username === 'admin' && password === 'admin5162') {
      const adminUser = {
        id: 'hardcoded-admin',
        username: 'admin',
        name: '관리자',
      ...
      // 백그라운드로 DB에도 admin 계정 동기화 시도 (실패해도 무시)
      setTimeout(() => bootstrapAdmin(true).catch(() => {}), 100);
